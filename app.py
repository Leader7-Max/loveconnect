...
def process_image(uploaded_file):
    ...
    try:
        ...
        img = img.convert("RGB")
        img.thumbnail((400, 400)) # Format carré compact
        buffer = BytesIO()
...
def render_avatar(photo_b64, initial):
    """Affiche soit la photo de profil, soit les initiales par défaut"""
    if photo_b64:
...
if st.session_state.logged_in:
    ...
    with tab_profile:  
        bio = user["bio"] or "Aucune présentation."  
        initial = str(user["first_name"])[0].upper() if user["first_name"] else "?"  
        avatar_html = render_avatar(user["photo"], initial)
        ...
        # Option de mise à jour de la photo
        ...
        if not profiles:  
            ...
            for i, profile in enumerate(profiles):  
                with cols[i % 2]:  
                    ...
                    avatar_html = render_avatar(profile["photo"], initial)
