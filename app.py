...
if st.session_state.logged_in:
    # Rafraîchissement automatique uniquement lorsque l'utilisateur est connecté
            ...
            with head_col2:
                with st.popover("⋯", use_container_width=True):
                    if st.button("🚫 Ne plus matcher", key=f"unmatch_{active_match['id']}", use_container_width=True):
                        ...
                        st.toast(f"Match supprimé avec {active_match['first_name']}.")
                        st.rerun()
                    if st.button("⛔ Bloquer", key=f"block_{active_match['id']}", use_container_width=True):
                        db.block_user(user["id"], active_match["id"])
                        st.session_state.active_chat_user_id = None
                        st.toast(f"{active_match['first_name']} a été bloqué(e).")
                        st.rerun()
                    if st.button("⚠️ Signaler", key=f"report_{active_match['id']}", use_container_width=True):
                        st.session_state.report_target = active_match['id']
                        st.rerun()

            if st.session_state.report_target == active_match['id']:
                with st.form(key="report_form"):
                    reason = st.text_input("Motif du signalement (faux profil, comportement inapproprié...)")
                    if st.form_submit_button("Envoyer le signalement"):
                        if reason.strip():
                            db.report_user(user["id"], active_match["id"], reason.strip())
                            db.block_user(user["id"], active_match["id"
