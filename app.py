...
if st.session_state["user_id"] is None:
  ...
  if users:
    ...
    for u in users:
      ...
      with cols[1]:
        # CORRECTION EFFECTUÉE ICI : {u[2]} ans au lieu de {u[2] ans}
        st.write(f"**{u[1]}**, {u[2]} ans - {u[3]}")
  else:
    st.info("Aucun membre inscrit pour le moment.")

else:
  st.title(f"Salut {current_user[1]} ! ❤️")
  st.write("Explore les profils et envoie des messages.")

  # Affichage des autres profils
  other_users = [u for u in users if u[0] != st.session_state["user_id"]]
    ...
    for u in other_users:
      ...
      with cols[0]:
        if u[4] and os.path.exists(u[4]):
          ...
          st.write("👤 Pas de photo")
      with cols[1]:
        st.write(f"### {u[1]}, {u[2]} ans")
        st.write(f"*{u[3]}*")
  else:
    st.info("Il n'y a pas d'autres utilisateurs pour le moment.")
