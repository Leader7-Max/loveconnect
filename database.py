...
def create_tables():
    ...
    # Table des likes (intérêts envoyés)
    conn.execute(
        """
    ...
    # Table des messages (messagerie privée)
    conn.execute(
        """
...
def send_like(sender_id, receiver_id):
    ...
    Retourne (True, is_match) si réussi, ou (False, False) si déjà liké.
    """
    conn = get_connection()
    try:
        conn.execute(
        ...
        # Vérification si l'autre utilisateur a aussi liké
        mutual = conn.execute(
            """
...
def get_matches(user_id):
    """Récupère les profils avec lesquels l'utilisateur a un match réciproque."""
    conn = get_connection()
    query = """
...
def get_messages(user_id_1, user_id_2):
    """Récupère la discussion complète entre deux utilisateurs."""
    conn = get_connection()
