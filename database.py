...
def format_last_seen(last_seen_str):
    ...
    try:
        ...
        if minutes < 5:
            ...
            return f'<span style="color:#94a3b8;font-size:11px;">⏱️ Vu il y a {minutes // 60}h</span>'
        else:
            return f'<span style="color:#94a3b8;font-size:11px;">⏱️ Vu il y a {minutes // 1440}j</span>'
    except Exception:
        return '<span style="color:#94a3b8;font-size:11px;">⏱️ Hors ligne</span>'
...
def update_profile(user_id, bio, country, city, looking_for):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE users SET bio = ?, country = ?, city = ?, looking_for = ? WHERE id = ?", (bio, country, city, looking_for, user_id))
    conn.commit()
    conn.close()
    return True
...
def get_profiles(current_user_id, country_filter="", city_filter="", min_age=None, max_age=None, respect_preferences=True):
    conn = get_connection()
    cursor = conn.cursor()
...
def unmatch(user1_id, user2_id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM likes WHERE (sender_id = ? AND receiver_id = ?) OR (sender_id = ? AND receiver_id = ?)", (user1_id, user2_id, user2_id, user1_id))
    conn.commit()
    conn.close()
    return True
...
def block_user(user_id, blocked_id):
    ...
    try:
        ...
        cursor.execute("DELETE FROM likes WHERE (sender_id = ? AND receiver_id = ?) OR (sender_id = ? AND receiver_id = ?)", (user_id, blocked_id, blocked_id, user_id))
        conn.commit()
        return True
    except sqlite3.IntegrityError:
        return False
...
def mark_messages_read(receiver_id, sender_id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE messages SET is_read = 1 WHERE receiver_id = ? AND sender_id = ? AND is_read = 0", (receiver_id, sender_id))
    conn.commit()
    conn.close()
...
def get_unread_count(receiver_id, sender_id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) as c FROM messages WHERE receiver_id = ? AND sender_id = ? AND is_read = 0", (receiver_id, sender_id))
    row = cursor.fetchone()
    conn.close()
